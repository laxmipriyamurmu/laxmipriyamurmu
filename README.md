# Hi, I'm Laxmipriya Murmu 👋

🚀 **Aerospace Engineering Undergraduate | Rocket Propulsion | Space Systems | CFD & Aerodynamics**

---

## About Me

I am a final-year Aerospace Engineering undergraduate passionate about Rocket Propulsion, Space Systems Engineering, Computational Fluid Dynamics (CFD), and Aerodynamics.

My experience spans hybrid rocket motor development, solid rocket propulsion systems, CubeSat subsystem integration, hypersonic CFD analysis, wind tunnel experimentation, and aerospace research.

I enjoy combining engineering fundamentals, simulation tools, and hands-on experimentation to develop practical aerospace solutions.

I am actively seeking opportunities in **propulsion, space systems, CFD, and aerospace engineering**.

---

## Technical Interests

* 🚀 Rocket Propulsion
* 🔥 Hybrid Rocket Systems
* 🛰️ Space Systems Engineering
* 📡 CubeSat Development
* ✈️ Computational Fluid Dynamics (CFD)
* 🛩️ Aerodynamics
* ⚙️ Aerospace Design & Analysis

---

## Technical Skills

### Aerospace & Propulsion

* Rocket Propulsion
* Hybrid Rockets
* Solid Propellant Rockets
* NASA CEA
* Combustion Analysis
* Aerodynamics
* Space Systems Engineering

### Simulation & Design

* ANSYS Fluent
* ANSYS Workbench
* Finite Element Analysis (FEA)
* CATIA V5
* Creo Parametric
* SolidWorks
* Fusion 360

### Programming & Analytics

* Python
* MATLAB
* C++
* Machine Learning
* Pandas
* NumPy
* Scikit-Learn

### Embedded Systems

* ESP32
* PCB Design
* Sensor Integration
* Telemetry Systems
* Firebase

---

## Featured Projects

🚀 **Hybrid Rocket Motor Development**
Design and performance evaluation of a 100 N-class hybrid rocket motor using paraffin-based fuel and liquid oxygen.

🚀 **E-Class Solid Propellant Rocket**
Development of a competition-winning E-class rocket with parachute recovery system.

🛰️ **LUMISAT – CubeSat for Artificial Light Pollution Monitoring**
1.5U CubeSat integrating sensing, telemetry, PCB development, and environmental monitoring systems.

✈️ **Hypersonic Vehicle CFD Analysis**
Mach 5 and Mach 7 aerodynamic performance analysis using ANSYS Fluent.

🛩️ **NACA 23012 Airfoil Design and Wind Tunnel Testing**
Design, fabrication, and experimental validation of a slotted NACA 23012 airfoil.

📄 **Transport Aircraft Selection Using Hybrid CRITIC-TOPSIS**
Springer-published research on transport aircraft selection under uncertainty using MCDM techniques.

---

## Experience

### Research Intern | Academy Innova World

* Developed Python-based aerospace data analytics models using aircraft design datasets.
* Performed regression modeling, feature engineering, and statistical analysis.
* Contributed to a Springer-published research paper on transport aircraft selection using CRITIC-TOPSIS and neutrosophic decision-making.

### 3D Printing & Finite Element Analysis Intern | HRDC, Lovely Professional University

* Designed aerospace components using Creo Parametric.
* Conducted structural analysis using ANSYS.
* Fabricated engineering prototypes using additive manufacturing technologies.

---

## Publication

### Selection of Transport Aircraft Using the Hybrid CRITIC–TOPSIS Method and Imputation of Missing Values in a Neutrosophic Environment

**International Journal of System Assurance Engineering and Management (Springer Nature), 2025**

DOI: **10.1007/s13198-025-02985-0**

---

## Achievements

🏆 2nd Runner-Up – 9th Innotek Annual Innovation Expo 2026

🚀 Winner – Cognitia 2025 E-Class Rocket Development Competition

📄 Co-author of a Springer-published research paper

🥈 Silver Medal – NCC Sangram 2024 Tug-of-War Competition

🏆 Overall Winner – NCC Sangram 2024 (Charlie Platoon)

---

## Current Focus

* Rocket Propulsion Systems
* Hybrid Rocket Development
* Space Systems Engineering
* CubeSat Technologies
* CFD & Aerodynamics
* Aerospace Product Development

---

## Connect With Me

🔗 LinkedIn: https://www.linkedin.com/in/laxmipriyamurmu/

💻 GitHub: https://github.com/laxmipriyamurmu

📧 Email: [laxmipriyam1405@gmail.com](mailto:laxmipriyam1405@gmail.com)

---

*"Engineering solutions for aerospace challenges through design, simulation, experimentation, and innovation."*
