# Hi, I'm Laxmipriya Murmu 👋

🚀 Aerospace Engineering Undergraduate at Lovely Professional University

## About Me

I am a final-year Aerospace Engineering undergraduate with interests in Rocket Propulsion, Space Systems Engineering, Computational Fluid Dynamics (CFD), and Aerodynamics.

My experience includes hybrid rocket motor development, solid rocket propulsion systems, CubeSat subsystem integration, hypersonic CFD analysis, wind tunnel experimentation, and aerospace data analytics using Python and Machine Learning.

I am actively seeking opportunities in propulsion, space systems, and aerospace engineering while continuing to develop practical solutions for aerospace applications.

---

## Technical Interests

* Rocket Propulsion
* Hybrid Rocket Systems
* Space Systems Engineering
* CubeSat Development
* Computational Fluid Dynamics (CFD)
* Aerodynamics
* Aerospace Design & Analysis

---

## Technical Skills

### Aerospace & Propulsion

Rocket Propulsion, Hybrid Rockets, Solid Propellants, NASA CEA, Combustion Analysis, Aerodynamics, Space Systems Engineering

### Simulation & Design

ANSYS Fluent, ANSYS Workbench, FEA, CATIA V5, Creo Parametric, SolidWorks, Fusion 360

### Programming & Analytics

Python, MATLAB, C++, Machine Learning, Pandas, NumPy, Scikit-Learn

### Embedded Systems

ESP32, PCB Design, Sensor Integration, Telemetry Systems, Firebase

---

## Featured Projects

🚀 Hybrid Rocket Motor Development

🚀 E-Class Solid Propellant Rocket

🛰️ LUMISAT – CubeSat for Artificial Light Pollution Monitoring

✈️ Hypersonic Vehicle CFD Analysis

🛩️ NACA 23012 Airfoil Design and Wind Tunnel Testing

📊 Aircraft Fuel Consumption Prediction using Machine Learning

---

## Publication

**Selection of Transport Aircraft Using the Hybrid CRITIC–TOPSIS Method and Imputation of Missing Values in a Neutrosophic Environment**

International Journal of System Assurance Engineering and Management (Springer Nature), 2025

DOI: 10.1007/s13198-025-02985-0

---

## Connect With Me

LinkedIn: [www.linkedin.com/in/laxmipriyamurmu](http://www.linkedin.com/in/laxmipriyamurmu)

Email: [laxmipriyam1405@gmail.com](mailto:laxmipriyam1405@gmail.com)
